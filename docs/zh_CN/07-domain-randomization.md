...
如果后续要在 MuJoCo 后端暴露原始 actuator PD 随机化，还需要先明确几个边界：

- 这套语义只对 MuJoCo 的 position actuator 成立，不能直接泛化到 `motor`、`velocity`、`damper`、`muscle` 或混合 actuator 模型
- 更严格地说，只有 `gaintype == mjGAIN_FIXED`、`biastype == mjBIAS_AFFINE`，且 `dyntype` 为 `mjDYN_NONE` 或 `mjDYN_FILTEREXACT` 的 actuator 才能按 position actuator 解释
- 对这类 actuator，`kp` 对应 `actuator_gainprm[:, 0]`，并且必须同步满足 `actuator_biasprm[:, 1] = -kp`
- 这里的 `kd` 实际对应 MuJoCo XML 里的 `kv`，即速度阻尼项；底层存储是 `actuator_biasprm[:, 2] = -kd`
- 如果 actuator 使用的是 `dampratio` 而不是 `kv`，那么 `actuator_biasprm[:, 2]` 的正值语义变成阻尼比，不应再解释为 `kd`
- 因此即使未来补 MuJoCo backend 实现，也需要 backend 在应用前按 actuator 类型逐个校验，而不是把 `kp/kd` 当成无条件通用项

也就是说，文档这一版不要把 `kp/kd` DR 视为“已统一支持”的能力，也不要把它写成 backend-agnostic 的公共 term。如果后面要支持这类随机化，需要先在 `src/unilab/dr/types.py` 里增加新的 reset term，再由各 backend 显式声明、校验并实现。
