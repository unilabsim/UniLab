...
3. 现在“统一”的主要是入口和执行流程，不是所有随机项本身。公共 helper [`build_common_reset_randomization()`](../../src/unilab/dr/dr_utils.py) 目前生成 `base_mass_delta`、`base_com_offset`、`kp`、`kd`；公共 interval helper 目前只生成 push。当前仓库没有统一的“direct external force / torque” interval contract。
...
| `SharpaInhandRotation` | grasp cache 按 `scale_ids` 从多份单-scale cache 采样；object pose / quat reset；可选 `base_mass_delta`；可选 `base_com_offset` | 无 | `scale_list` 默认 `[0.5]`，MuJoCo 下会在 init 阶段 materialize object geom scale |
| `SharpaInhandRotationGrasp` | hand pose reset；object pose / quat reset；采集成功 grasp 并保存为单个 scale 的 cache 文件；可选 `base_mass_delta`；可选 `base_com_offset` | 无 | 默认用于生成 Sharpa grasp cache；每次运行只采一个 scale，对应一个 cache 文件 |
...
- interval common payload：push，和 MuJoCo 专用的 `body_linear_velocity_delta`
...
- [`MuJoCoBackend`](../../src/unilab/base/backend/mujoco_backend.py)：支持上面 6 个 reset term，且支持 interval push，以及 MuJoCo 专用的 `body_linear_velocity_delta`
...
```bash
uv run scripts/train_rsl_rl.py task=sharpa_inhand/mujoco 'env.scale_list=[0.5,0.6,0.7,0.8]' algo.num_envs=4096
```
...
```bash
uv run scripts/train_rsl_rl.py task=sharpa_inhand_grasp/mujoco 'env.scale_list=[0.5]' algo.num_envs=4096
uv run scripts/train_rsl_rl.py task=sharpa_inhand_grasp/mujoco 'env.scale_list=[0.6]' algo.num_envs=4096
uv run scripts/train_rsl_rl.py task=sharpa_inhand_grasp/mujoco 'env.scale_list=[0.7]' algo.num_envs=4096
uv run scripts/train_rsl_rl.py task=sharpa_inhand_grasp/mujoco 'env.scale_list=[0.8]' algo.num_envs=4096
```
