...
`sharpa_inhand` 的完整 HORA 流程仍以 MuJoCo 为 owner。Motrix 目前提供 phase-1 PPO rotation 支持：`task=sharpa_inhand/motrix` 可以运行 reset/step 与最小训练 smoke，并支持 object scale variants、object mass/COM randomization、geom friction randomization、direct object `body_force` interval disturbance 和 position actuator 的 `kp/kd` reset randomization；但它不是完整 HORA/MuJoCo 能力等价路径。

Motrix phase-1 仍显式关闭完整 Sharpa DR 中尚未支持的项：gravity direction randomization。`SharpaInhandRotationGrasp` grasp cache 生成支持 MuJoCo 和 Motrix owner。

### 配置文件

- `scripts/train_rsl_rl.py` 主配置：`conf/ppo/config.yaml`
- `task=sharpa_inhand_grasp/mujoco`：`conf/ppo/task/sharpa_inhand_grasp/mujoco.yaml`
- `task=sharpa_inhand_grasp/motrix`：`conf/ppo/task/sharpa_inhand_grasp/motrix.yaml`，继承 `conf/ppo/task/sharpa_inhand_grasp/mujoco.yaml`
- `task=sharpa_inhand/motrix`：`conf/ppo/task/sharpa_inhand/motrix.yaml`，继承 `conf/ppo/task/sharpa_inhand/mujoco.yaml` 并关闭未支持的 DR 项
...
Motrix grasp cache 生成使用同一任务配置结构，只需要切换 owner：

```bash
uv run scripts/train_rsl_rl.py task=sharpa_inhand_grasp/motrix 'env.domain_rand.scale_list=[1.0]' env.grasp_collection_target=1000 training.no_play=true
```
...
Motrix phase-1 使用同一组 `scale_list`，因此需要每个 scale bucket 对应的 cache 文件。例如默认配置会读取 `cache/sharpa_grasp_linspace_0.8.npy` 到 `cache/sharpa_grasp_linspace_1.6.npy` 的多份 cache；自定义 prefix 也遵循同样的 `<prefix>_<scale>.npy` 规则：
...
Motrix phase-1 PPO rotation:

```bash
uv run scripts/train_rsl_rl.py task=sharpa_inhand/motrix training.no_play=true
```
