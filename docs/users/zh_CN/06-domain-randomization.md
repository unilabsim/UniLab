...
4. [`ResetRandomizationPayload`](../../../src/unilab/dr/types.py) 已经能表达 `gravity`、`body_iquat`、`body_inertia`、`body_mass`、`body_ipos`、`kp`、`kd`，且 [`MuJoCoBackend`](../../../src/unilab/base/backend/mujoco/backend.py) 已声明支持。是否真正使用这些项，仍取决于任务 provider 是否采样并下发。
5. [`MotrixBackend`](../../../src/unilab/base/backend/motrix/backend.py) 当前支持 `base_mass_delta`、`base_com_offset`、`body_mass`、`body_ipos`、`geom_friction`、`kp`、`kd` 和 interval push；并支持 init-lifecycle geom size override。`kp/kd` 要求模型 actuator 全部为 position actuator。
...
| `SharpaInhandRotation` | grasp cache 按 `scale_ids` 分桶采样；object pose / quat reset；可选 common reset randomization payload（含 `gravity`）；可选 geom friction；可选 object mass/COM | object `body_force` direct force disturbance | `domain_rand.scale_list` 默认来自 owner YAML，MuJoCo / Motrix 下会在 init 阶段 materialize object geom scale；MuJoCo / Motrix owner 默认开启 geom friction 与 object force |
...
- [`MuJoCoBackend`](../../../src/unilab/base/backend/mujoco/backend.py)：支持上面这些 reset term，且支持 interval push 与 interval body force
- [`MotrixBackend`](../../../src/unilab/base/backend/motrix/backend.py)：支持 `base_mass_delta`、`base_com_offset`、`body_mass`、`body_ipos`、`geom_friction`、`kp`、`kd`，且支持 init-lifecycle geom size override、interval push 与 interval `body_force`；`kp/kd` 初始化阶段要求 actuator 全为 position
...
- Motrix backend 通过 `Link.add_external_force(..., local=False)` 支持 direct body-force disturbance；由于 Motrix API 是 add 语义，backend 会按 link 维护当前已写入 force，并用 delta 写入，保持 `IntervalRandomizationPlan.body_force` 的 absolute-force contract。
...
Motrix backend 使用同一个 `SceneModel`，但会在 init-lifecycle 阶段根据 env-to-scale assignment 预计算每个 env 的 geom size override，并在 reset 时把对应 override 写入 `SceneData`。这个路径不重新解析 XML，也不在 step 热路径读取 asset metadata。
...
Motrix owner 使用相同的单 scale 采集规则：

```bash
uv run scripts/train_rsl_rl.py task=sharpa_inhand_grasp/motrix 'env.domain_rand.scale_list=[1.0]' env.grasp_collection_target=1000 algo.num_envs=4096
```
...
- MuJoCo 通过 scale-specific `MjModel` 池表达 `geom_size`，Motrix 通过 per-env geom size override 表达 `geom_size`。
