...
A minimal `reward_config` stub is auto-generated by reflecting on the task's
RewardConfig dataclass — visualization doesn't need real reward scales.

Mujoco backend: stitches all `--num_envs` robot replicas into the same scene
(the scene the backend is actually using, which includes any procedural
terrain) and drives every replica's qpos/qvel each frame from
`env.get_physics_state_snapshot()`. Replica spawn positions are computed
with `compute_env_origins_grid(num_envs, env.cfg.env_spacing)` — the same
public utility the env uses internally.
...
Usage:
    ...
    uv run scripts/visualize_task_env.py --task Go1JoystickFlat --num_envs 4 --env_spacing 1.5
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Override cfg.env_spacing (meters). If unset, uses the cfg dataclass "
            "default (Go2 = 1.0, Go1/base = 0.0). Pass a positive value to spread "
...
def _run_mujoco(env, num_envs: int) -> None:
    ...
    env_spacing = float(getattr(env.cfg, "env_spacing", 0.0) or 0.0)
    env_origins = compute_env_origins_grid(num_envs, env_spacing)

    if num_envs > 1 and env_spacing == 0.0:
        print(
            "[visualize_task_env] WARNING: cfg.env_spacing == 0; all "
            f"{num_envs} robots will overlap. Pass --env_spacing 1.5 (or "
            "similar) to spread them out."
        )
...
def _build_env_cfg_override(task_name: str, env_spacing: float | None) -> dict[str, Any]:
    ...
    if env_spacing is not None:
        override["env_spacing"] = float(env_spacing)
    return override
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(
        ...
        f"num_envs={args.num_envs} env_spacing={args.env_spacing}"
    )

    env_cfg_override = _build_env_cfg_override(args.task, args.env_spacing)
    env = registry.make(
        args.task,
