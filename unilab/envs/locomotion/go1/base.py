...
class Go1BaseMjEnv(MjNpEnv):
    ...
    def _init_buffer(self):
        ...
        if key_id >= 0:
            ...
            # Fallback to qpos0 if home is not defined
            self.default_angles = self._model.qpos0[7:].astype(np.float32)
    ...
    def _init_sensor_indices(self):
        ...
        # Go1 Specific Sensor Naming: "{prefix}_floor_found"
        prefixes = ["FL", "FR", "RL", "RR"]
        expected_names = [f"{p}_floor_found" for p in prefixes]
        ...
        if "position" in self.sensor_indices:
             self.idx_global_pos = self._get_sensor_indices("position")
        elif "global_position" in self.sensor_indices:
        ...
        for name in foot_names:
            sname = f"{name}_pos"
            if sname in self.sensor_indices:
